# NOVAMANYA ТСД — отгрузка и сборка Kaspi

Приложение операторов склада на ТСД Mindeo. С 2026-08-31 работает с
**прод-ядром novamanya `https://qoimams.asia/api/`** под единым логином ядра:

- **Отгрузка** (`/tsd/*`): смена → скан посылок (лок между ТСД на сервере) →
  «Создать отгрузки» — отгрузка фиксируется **в ядре** (`shipped_at`), МойСклад
  больше не используется. История смен и деталка — тоже из ядра.
- **Сборка** (`/picker/*`): рабочее место сборщика, те же задания, что у
  веб-сборщика. Дизайн скопирован с waybills: вкладки «Задания / История»,
  «СЕЙЧАС ИЩЕМ», режим «скан + кол-во» для массовых, частичное завершение
  (недобранное → очередь отмены), повторная печать из истории, ячейки 📍 и фото.
- **Инвентаризация** — ЕДИНСТВЕННОЕ, что осталось на старом wms-backend
  (`194.238.41.18`), со своим отдельным логином (`WmsLoginActivity`); в ядре
  инвентаризации пока нет.

Скан — аппаратной пикалкой (BroadcastReceiver Mindeo/NLS/Sunmi/Honeywell +
клавиатурный ввод, суффикс `-N` Mindeo обрезается). Статусы сканов решает
сервер. Ключевые файлы: `api/CoreApi*.kt` (клиент ядра), `CoreAuth.kt` (токен),
`MainActivity` (отгрузка), `PickerTasksActivity`/`PickerTaskActivity` (сборка),
`HistoryActivity`/`SessionDetailActivity` (история смен отгрузки).

> Ниже — историческое описание старой архитектуры (wms-backend + МойСклад).
> Актуально только для раздела «Инвентаризация».
