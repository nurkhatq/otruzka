...
## Раздел «Сборка» (сборщик заказов, 2026-08-31)

Отдельный раздел в этом же приложении — рабочее место сборщика на ТСД. Ходит
**не** в старый wms-бэкенд, а в прод-ядро novamanya **https://qoimams.asia/api/**
(эндпоинты `/picker/*` — те же, что у веб-сборщика). Скан — аппаратной пикалкой
(BroadcastReceiver + клавиатурный ввод), камера не используется.

- Вход: кнопка «Сборка» на главном экране или «Я сборщик» на экране логина.
  Аккаунт — из ядра qoimams (тот же, что для веб-сборщика); логин отгрузки не нужен.
- `PickerLoginActivity` — вход, выбор магазина (novamanya/duglas) и города.
- `PickerTasksActivity` — смена (начать/завершить), список назначенных заданий
  (round-robin раздаёт сервер).
- `PickerTaskActivity` — скан позиций: статус скана решает сервер
  (совпал / неизвестный ШК / без ШК); режим «скан + кол-во» для массовых;
  частичное завершение → недобранное в очередь отмены; ячейки 📍 и фото товара.
- `PickerHistoryActivity` — история сборок (за смену / вся), повторная печать
  накладной на принт-станцию.
- Клиент/токен: `api/CoreApi*.kt`, `CoreAuth.kt` (хранится отдельно от wms_auth).
